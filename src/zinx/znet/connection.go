...
func (c *Connection) StartWriter() {
	...
	zlog.Debug("[Writer Goroutine is running]")
	defer zlog.Debug(c.RemoteAddr().String(), "[conn Writer exit!]")
...
func (c *Connection) SendMsg(m ziface.IMessage) error {
	if c.isClosed == true {
	...
	msg, err := dp.Pack(m)
	if err != nil {
		zlog.Error("Pack error msg id = ", m.GetMsgId())
...
func (c *Connection) SendBuffMsg(m ziface.IMessage) error {
	if c.isClosed == true {
